...
class PriceTracker(ABC):
    def write_to_database(self):
        ...
        content = f"{now} --> {self.__class__.__name__} --> {self.price}"

        # Assuming you have an SQLite database named 'prices.db'
        conn = sqlite3.connect('prices.db')
        cursor = conn.cursor()

        cursor.execute("INSERT INTO tracked_prices (product_name, timestamp, platform, price) VALUES (?, ?, ?, ?)",
                       (self.product_name, now, self.__class__.__name__, self.price))
